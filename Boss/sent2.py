...
def handle_search(update, context):
    keyboard = [
        ...
        [InlineKeyboardButton("By email address",
                              callback_data='search_email')],
    ...
    query.edit_message_text(
        text="Please choose a search criterion:", reply_markup=reply_markup)


def handle_search_query(update, context):
    if not context.user_data.get('waiting_for_search_query'):
        return
    search_type = context.user_data.pop('search_type', None)
    context.user_data['waiting_for_search_query'] = False
    if search_type == 'subject':
        # Rename this function accordingly
        handle_subject_input(update, context)
    elif search_type == 'email':
        handle_email_input(update, context)  # Rename this function accordingly
...
def handle_search_subject(update, context):
    ...
    query.answer()
    query.edit_message_text(
        text="Please enter the subject to search for:", reply_markup=reply_markup)
    context.user_data['search_type'] = 'subject'
    context.user_data['waiting_for_search_query'] = True  # Add this line
...
def handle_search_email(update, context):
    ...
    query.answer()
    query.edit_message_text(
        text="Please enter the email address to search for:", reply_markup=reply_markup)
    context.user_data['search_type'] = 'email'
    context.user_data['waiting_for_search_query'] = True  # Add this line


# This was previously handle_subject_search
def handle_subject_input(update, context):
    ...
    if df.empty:
        ...
        for _, row in df.iterrows():
            message += f"{row['user_name']} sent {row['subject']} to {row['to_email']} at {row['timestamp'].strftime('%d.%m.%Y %H:%M')}\n"
        update.message.reply_text(message.strip())


def handle_email_input(update, context):  # This was previously handle_email_search
...
def sent(update, context):
    keyboard = [
        ...
        [InlineKeyboardButton("Last 50", callback_data='summary_4')],
...
def generate_summary(update, sent_emails, summary_type):
    ...
    if summary_type == 'summary_1':
        ...
        df_last_two_weeks['subject'].value_counts().plot(
            kind='bar', ax=axes[0], color=color_scheme)
        ...
        df['week_number'].value_counts().sort_index().plot(
            kind='bar', ax=axes[1], color=color_scheme)
        ...
        for _, row in last_entries.iterrows():
            text_message += f"{row['user_name']} sent {row['subject']} to {row['to_email']} at {row['timestamp'].strftime('%d.%m.%Y %H:%M')}\n"

        return text_message

    elif summary_type == 'summary_4':
        # Just display the 50 last entries from the JSON file.
        last_entries = df.tail(50)
        if last_entries.empty:
            return "There are no recent email entries."
        text_message = ""
        for _, row in last_entries.iterrows():
...
def handle_summary(update, context):
    ...
    if summary_type in ['summary_2', 'summary_4']:
        context.bot.send_message(
            chat_id=update.effective_chat.id, text=summary_response)
    elif summary_type == 'summary_3':
        context.bot.send_document(
            chat_id=update.effective_chat.id, document=summary_response)
    else:
        context.bot.send_photo(
            chat_id=update.effective_chat.id, photo=summary_response)
...
def handle_back_to_menu(update, context):
    ...
    query.edit_message_text('Please choose an option:',
...
def main3():
    ...
    dp.add_handler(MessageHandler(Filters.text & ~
                   Filters.command, handle_search_query))
    dp.add_handler(CallbackQueryHandler(
