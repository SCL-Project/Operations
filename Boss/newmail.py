...
from config import BOT_TOKEN, ELASTIC_API_KEY, STRATO_EMAIL_ADDRESS, HTML_TEMPLATE_AFTER, HTML_TEMPLATE_BEFORE


def newmail(update: Update, context: CallbackContext):
    update.message.reply_text(
        "Please enter the receiver's email address, subject, CC and BCC addresses separated by commas:")
    context.user_data['state'] = 'newmail_info'  # setting the initial state
...
def newmail_info_received(update: Update, context: CallbackContext):
    ...
    if state == 'newmail_info':
        ...
        # If the user has not entered CC and BCC addresses, default them to an empty string
        context.user_data['email'], context.user_data['subject'] = info[0].strip(
        ), info[1].strip()
        context.user_data['cc'] = info[2].strip() if len(info) > 2 else ""
        context.user_data['bcc'] = info[3].strip() if len(info) > 3 else ""
        ...
        # change the state to expect the email body next
...
def newmail_body_received(update: Update, context: CallbackContext):
    ...
    cc_email = context.user_data['cc']
    bcc_email = context.user_data['bcc']
...
def newmail_send_email(update: Update, context: CallbackContext):
    ...
    cc_email = context.user_data['cc']  # CC address
    bcc_email = context.user_data['bcc']  # BCC address
    ...
    data = {
        ...
        "msgto": to_email,
        "msgcc": cc_email,  # Add CC here
        "msgbcc": bcc_email,  # And BCC here
        "subject": subject,
        "bodyHtml": email_body,
...
def show_email_preview(update: Update, context: CallbackContext):
    ...
    email_body_text = re.sub(
        r'.*!\[Header Image\]\(https://i.imgur.com/G9NkCBX.png\).*', '', email_body_text)

    email_body_text = re.sub(r'---', '', email_body_text)
    email_body_text = re.sub(r'# Smart Contracts Lab', '', email_body_text)
    email_body_text = re.sub(
        r'\(C\) 2023 Smart Contracts Lab', '', email_body_text)
